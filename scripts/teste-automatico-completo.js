...
async function marcarComoRecebida(paymentId) {
...
async function verificarSupabase(paymentId) {
  ...
  const queryPagamento = `
    select: id,tipo,valor_bruto,valor_liquido,status,competencia,asaas_payment_id,asaas_event_id,pago_em,created_at,cliente_id,clientes(nome_empresa,asaas_customer_id,contador_id)
  `;
  ...
  if (!pagamento) {
    ...
    const responseUltimos = await fetch(
      `${SUPABASE_URL}/rest/v1/pagamentos?select=id,asaas_payment_id,created_at&order=created_at.desc&limit=5`,
      {
        headers: {
          'apikey': SUPABASE_SERVICE_ROLE_KEY,
          'Authorization': `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
    ...
    const ultimos = await responseUltimos.json();
    console.log('Últimos 5 pagamentos:', JSON.stringify(ultimos, null, 2));
...
function gerarRelatorio(paymentId, resultado) {
  ...
  console.log('\n🔗 LINKS ÚTEIS:');
  console.log(`  • Supabase Dashboard: https://supabase.com/dashboard/project/zytxwdgzjqrcmbnpgofj`);
